...
constexpr uint8_t NUM_MOVES = 10;
constexpr uint8_t CAN_TURN_AT_REMAINING_MOVES = NUM_MOVES - 4;
...
struct CrucibleState {
    ...
    CrucibleState(const size_t heat_loss, const Point& position, const size_t distance_to_goal, const Point& direction, const uint8_t remaining_moves, const std::vector<Point> &debug)
        : heat_loss(heat_loss),
          position(position),
          distance_to_goal(distance_to_goal),
          direction(direction),
    remaining_moves(remaining_moves), debug(debug) {}

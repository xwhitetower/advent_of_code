...
fn move_cargo(cargo: &mut Vec<Vec<char>>, instructions: &[String]) {
    for instruction in instructions {
        let instruction_data: Vec<usize> = instruction.split(' ')
            ...
            .collect();
        for _ in 0..instruction_data[0] {
            let container = cargo[instruction_data[1] - 1].pop().unwrap();
            cargo[instruction_data[2] - 1].push(container);
        }
    }
}
...
fn main() -> Result<(), Box<dyn Error>> {
    let lines = read_lines("./data/day5/input.txt")?;
...
fn test() {
    ...
    let expected_lines = read_lines("./data/day5/part1/test_result.txt").unwrap();

...
fn trim(map: &mut Map) {
    while map[0].iter().all(|x| x == &EMPTY) {
        map.pop_front();
    }
    while map[map.len() - 1].iter().all(|x| x == &EMPTY) {
        map.pop_back();
    }
    while map.iter().all(|x| x[0] == EMPTY ) {
        map.iter_mut().for_each(|x| { x.pop_front(); } );
    }
    while map.iter().all(|x| x[x.len() - 1] == EMPTY ) {
        map.iter_mut().for_each(|x| { x.pop_back(); } );
    }
}
...
fn test() {
    ...
    let expected_lines = read_lines("./data/day23/part1/test_result.txt").unwrap();

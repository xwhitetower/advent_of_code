...
fn solve(lines: Vec<String>) -> u32 {
    ...
    let start = (1, 0);
    let end = (x_size - 2, y_size - 1);
    let mut map = vec![vec![false; x_size]; y_size];
    let mut minute =  trip(1, start, end, x_size, y_size, &blizzards);
    let mut minute =  trip(minute, end, start, x_size, y_size, &blizzards);
    trip(minute, start, end, x_size, y_size, &blizzards)
}
...
fn trip(start_minute: u32, from: (usize, usize), to: (usize, usize), x_size: usize, y_size: usize, blizzards: &Blizzards) -> u32 {
    let mut map = vec![vec![false; x_size]; y_size];
    let mut minute = start_minute;
    loop {
        if to.1 == 0 && map[to.1 + 1][to.0] || to.1 == y_size - 1 && map[to.1 - 1][to.0] {
            return minute;
        }
        map[from.1][from.0] = true;
        map = paint_map(&map);
...
fn test() {
    ...
    let expected_lines = read_lines("./data/day24/part2/test_result.txt").unwrap();

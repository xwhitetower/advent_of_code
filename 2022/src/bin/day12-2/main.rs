      ...
      $pos_height: expr, $boundary_check:expr, $lazy_pos: expr) => {
        if $boundary_check {
            ...
            if can_traverse(&$map, &$visited, $pos_height, new_pos) {
                if $map[new_pos.0][new_pos.1] == START_VALUE || $map[new_pos.0][new_pos.1] == START_HEIGHT {
...
fn solve(lines: Vec<String>) -> u32 {
    ...
    let end = find_end_position(&map);
    visited[end.0][end.1] = 0;
    visit_queue.push_back(end);
...
fn find_end_position(map: &HeightMap) -> Point {
    for i in 0..map.len() {
...
fn can_traverse(map: &HeightMap, visited: &VisitedMap, height: char, coord: Point) -> bool {
    let pos_value = position_value(&map, coord);
    visited[coord.0][coord.1] == NOT_VISITED
        && (height == pos_value || char::from_u32(height as u32 - 1).unwrap() == pos_value || height < pos_value)
}
...
fn test() {
    ...
    let expected_lines = read_lines("./data/day12/part2/test_result.txt").unwrap();
